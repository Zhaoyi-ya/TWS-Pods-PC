"""
Huawei 蓝牙传输层 —— HyperEars Python 移植（仅华为）

Huawei FreeBuds 的私有控制通道是 **Bluetooth Classic RFCOMM (SPP)**
（UUID 00001101-0000-1000-8000-00805F9B34FB），**不是 BLE**。

连接与枚举方式（参考用户 OPPO-Pods-Win 原型，已在本机验证可行）：
...
- ``SimulatedTransport``：纯软件模拟 FreeBuds 3 设备，按真实协议帧/AT 行回包。
  用于在无硬件 / Windows 上演示完整管线（默认通道）。
...
from huawei_models import select_profile
...
class SimulatedTransport(Transport):
    """模拟一台 HUAWEI FreeBuds 3 设备。

    - 电量：收到 ``AT+HUAWEIBATTERY?`` 时回一行 ``+HUAWEIBATTERY: ...``（与真实 HFP AT 行一致）。
    - 降噪 / 手势：私有帧为 fire-and-forget，模拟器不回包（与参考项目行为一致）。
    """

    def __init__(
        self,
        address: str = "AA:BB:CC:00:11:22",
        device_name: Optional[str] = "HUAWEI FreeBuds 3",
        profile=None,
    ...
    def send(self, data: bytes) -> None:
        ...
        if data == BATTERY_QUERY:
            self._pending.append(self._battery_line())

    def _battery_line(self) -> bytes:
        l, r, c = self._battery
        flags = (
        ...
        line = (
            f"+HUAWEIBATTERY: 6,2,{l},3,{flags & 1},4,{r},5,{(flags >> 1) & 1},"
            f"6,{c},7,{(flags >> 2) & 1}"
        )
        return (line + "\r\n").encode("utf-8")
...
class PyBluezRfcommTransport(Transport):
    ...
    def connect(self) -> None:
        ...
        if self.channel is None:
            ...
            if not self.channel:
                raise RuntimeError(f"未找到 Huawei RFCOMM 服务（UUID={self.uuid}）")
...
def _uuid_to_guid(uuid_str: str) -> _GUID:
    """把 '00001101-0000-1000-8000-00805F9B34FB' 转成 Windows GUID 结构。"""
...
class WinSockRfcommTransport(Transport):
    ...
    连接采用 Winsock 的隐式 SDP：``port=0`` 且 ``serviceClassId`` 设为 SPP UUID
    时，``connect()`` 会自动做 SDP 查询找到通道。若需指定固定通道，可传 ``channel=``。

    要求：耳机已在 Windows 系统里**配对**（设置 → 蓝牙 → 已配对）。
    控制帧通过 SPP UUID ``00001101-0000-1000-8000-00805F9B34FB`` 收发。
    """
...
# '484u51574549204672656542756473203300' -> 'HUAWEI FreeBuds 3'。先试 Name，再试 LEName。
_NAME_VALUES = ("Name", "LEName")


def _decode_reg_name(raw) -> str:
    """注册表设备名可能是：
    - REG_BINARY：原始 UTF-8 字节，如 b'HUAWEI FreeBuds 3\\x00'
    - REG_SZ：有时存成「UTF-8 字节的十六进制字符串」，如 '484...00'
    两种都归一化为可读名字。
    """
