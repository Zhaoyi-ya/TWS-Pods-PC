...
连接能力：
- 「连接模拟设备」：SimulatedTransport，离线即可演示完整管线。
- 「真实连接（RFCOMM）」：WinRfcommTransport，基于 Windows WinRT（需 pip install winrt
  且耳机已在系统配对）。
- 「扫描附近vivo耳机」：bleak 真实 BLE 扫描识别；双击列表项优先用真实 RFCOMM 连接。

依赖：仅标准库（tkinter）。可选：bleak（真实扫描）、winrt（Windows 真实 RFCOMM）。
"""
...
from vivo_models import is_family_name
from vivo_protocol import NoiseMode
...
class SessionCard(ttk.Frame):
    ...
    def _build(self):
        ...
        for key, label in (("L", "左耳"), ("R", "右耳"), ("C", "充电盒")):
            ...
            val = ttk.Label(row, text="—", width=10, style="Muted.TLabel")
            val.pack(side="left")
        ...
        self._mode_btns = {}
        for mode in (NoiseMode.ANC, NoiseMode.OFF, NoiseMode.TRANSPARENCY):
            btn = ttk.Button(ctrl, text=mode.name, width=8, command=lambda m=mode: self.app.on_set_noise(self.session, m))
...
class HyperEarsApp:
    def __init__(self, root: tk.Tk):
        ...
        self.sessions: Dict[str, Tuple[EarbudSession, SimulatedTransport, str]] = {}
    ...
    def _build_ui(self):
        ...
        self.vivo_only = tk.BooleanVar(value=True)
        ttk.Checkbutton(bar, text="仅 vivo/iQOO", variable=self.vivo_only).pack(side="left", padx=(0, 6))
    ...
    def on_connect_sim(self):
        addr, name = "AA:BB:CC:00:11:22", "vivo TWS Air3 Pro"
        ...
        self.set_status("已连接模拟设备（vivo TWS Air3 Pro）。可点降噪三态切换，或「刷新」重新查询。")
    ...
    def on_refresh(self, session: EarbudSession):
        ...
        for widget in self.cards.winfo_children():
            if isinstance(widget, SessionCard) and widget.session is session:
                widget.refresh()
        self.set_status(f"已刷新：{session.display_name}")

    def on_set_noise(self, session: EarbudSession, mode: NoiseMode):
        _, transport, _ = self.sessions.get(session.address, (None, None, None))
        if transport is None:
            return
        ...
        for widget in self.cards.winfo_children():
            if isinstance(widget, SessionCard) and widget.session is session:
                widget.refresh()
        self.set_status(f"已发送：{session.display_name} → {mode.name}")
    ...
    def _enum_thread(self):
        ...
        if self.vivo_only.get():
            found = [(n, m) for n, m in all_found if is_family_name(n)]
        else:
            found = all_found
        note = ""
        # 过滤后为空但原始有设备：自动回退显示全部，避免漏掉用户设备
        if self.vivo_only.get() and not found and all_found:
            found = all_found
            note = "（型号名未命中 vivo 关键词，已显示全部已配对设备）"
        self.devices = found
    ...
    def _update_device_list(self, found: List[Tuple[str, str]], note: str = ""):
        ...
        if not found:
            hint = (
                "未发现任何已配对设备。"
                + ("（取消「仅 vivo/iQOO」可看全部；或先在 Windows 设置里配对耳机）"
                   if self.vivo_only.get() else "（先在 Windows 设置里配对耳机）")
            )
